 ...
 */
export function computeStorageLayout(stateVariables) {
    const slots = [];
    let currentSlot = 0;
    let currentOffset = 0;
...
export function resolveAccessSlot(access, slots) {
 ...
 */
export function detectConflict(access1, access2) {
 ...
 */
export function formatStorageLayout(slots) {
    const lines = ["=== Solidity Storage Layout ===\n"];
    ...
    for (const slotNum of Array.from(slotGroups.keys()).sort((a, b) => a - b)) {
        ...
        for (const slot of slotVars) {
            const typeStr = slot.isMapping || slot.isArray ? `${slot.type} (ptr)` : slot.type;
            lines.push(`  ${slot.variable}: ${typeStr} (@offset ${slot.offset}, size ${slot.size}B)`);
        }
        lines.push("");
    }
    return lines.join("\n");
