 ...
 */
import { computeStorageLayout, detectConflict, resolveAccessSlot, } from "./storageLayout.js";
/**
 ...
 */
export function buildConflictMatrix(contract) {
    ...
    const stateModifyingFunctions = contract.functions.filter((f) => (f.visibility === "external" || f.visibility === "public") &&
        f.isStateModifying);
    const matrix = [];
 ...
 */
export function formatConflictMatrixTable(conflictMatrix) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j < n; j++) {
            const cell = matrix.find((c) => (c.function1 === functionNames[i] &&
    ...
    lines.push("Legend: 🔴 = Definite Conflict | 🟡 = Symbolic | ✓ = Safe | ─ = Self-write\n");
 ...
 */
export function formatConflictMatrixDetailed(conflictMatrix) {
    ...
    if (definite.length > 0) {
        ...
        for (const cell of definite) {
            ...
            for (const conflict of cell.conflicts) {
                ...
                lines.push(`  🔴 ${type1}(${conflict.slot1}) vs ${type2}(${conflict.slot2})`);
 ...
 */
export function analyzeParallelRisk(conflictMatrix, concurrentCalls) {
    ...
    for (const call1 of concurrentCalls) {
        for (const call2 of concurrentCalls) {
            if (call1.function === call2.function && call1.count > 1) {
                ...
                const cell = matrix.find((c) => (c.function1 === call1.function &&
                ...
                if (cell) {
                    ...
                    if (cell.overallSeverity === "SYMBOLIC")
                        hasSymbolicConflict = true;
                }
            }
        }
    }
...
//# sourceMappingURL=conflictMatrix.js.map