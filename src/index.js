 ...
 */
app.post("/monad/analyze", async (req, res) => {
    ...
    try {
        ...
        const verdict = determineVerdict(portabilityReport.summary.errors, conflictReport.summary.definiteConflicts);
        const recommendation = generateRecommendation(verdict, portabilityReport.summary.errors, conflictReport.summary.definiteConflicts);
        const response = {
            success: true,
            ...
            parallelization: {
                ...
                matrix: conflictReport.matrix.map((cell) => ({
 ...
 * - Otherwise → READY
 */
function determineVerdict(anchorErrors, definiteConflicts) {
    if (anchorErrors > 0)
        return "NOT_READY";
    if (definiteConflicts > 0)
        return "CAUTION";
    return "READY";
}
function generateRecommendation(verdict, anchorErrors, definiteConflicts) {
    if (verdict === "READY") {
