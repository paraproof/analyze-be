...
export interface PortabilityIssue {
  ...
  category:
    ...
    | "DELEGATECALL"
    | "SELFDESTRUCT";
  isAnchor: boolean; // NEW — true for the 6 core categories, false for extras
  line?: number;
  column?: number;
  code: string;
  message: string;
...
export interface MonadCompatibilityReport {
  ...
  checks: {
    total: number; // always 6 — the anchor categories only
    passed: number; // anchor categories with zero issues
    failed: number; // anchor categories with at least one issue
  };
  summary: {
    errors: number; // always from anchor categories only (extras can't be ERROR)
    warnings: number; // from anchor categories only (extras can't be WARNING)
    infos: number; // combined: anchor infos + all extra findings
  };
}
 ...
 */
export function checkMonadCompatibility(
  contract: ContractSummary,
): MonadCompatibilityReport {
  ...
  const warningCount = anchorIssues.filter(
    (i) => i.severity === "WARNING",
  ).length;
  // infos = anchor infos + ALL extra findings (extras are always INFO)
  const infoCount =
    anchorIssues.filter((i) => i.severity === "INFO").length +
    issues.filter((i) => !i.isAnchor).length;

  const anchorCategoriesWithIssues = new Set(
    anchorIssues.map((i) => i.category),
  );
...
function detectHardcodedGas(code: string): PortabilityIssue[] {
  const issues: PortabilityIssue[] = [];
...
function detectGasRefundLogic(code: string): PortabilityIssue[] {
  const issues: PortabilityIssue[] = [];

  const refundRegex =
    /refund\s*=.*(?:tx\.gasprice|gasRefund|21000|20000|SELFDESTRUCT)/gi;
  let match;
 ...
 */
function detectGasReimbursement(code: string): PortabilityIssue[] {
  const issues: PortabilityIssue[] = [];

  const gasUsedDeltaRegex =
    /(?:uint256|uint)\s+\w*[Gg]as[Uu]sed\w*\s*=\s*\w+\s*-\s*gasleft\(\)/g;
  let match;
  ...
  const reimbursementTransferRegex =
    /\.(?:transfer|send|call)\s*(?:\{[^}]*\})?\s*\(\s*\w*[Gg]as\w*\s*\*\s*tx\.gasprice/g;
  ...
  const relayerKeywords =
    /(relayer|paymaster|refundGas|reimburseGas|gasStation)/i;
 ...
 */
function detectCreate2Eip7702(code: string): PortabilityIssue[] {
  const issues: PortabilityIssue[] = [];

  const hasCreate2 = /create2\s*\(/i.test(code);
  if (!hasCreate2) return issues;
  ...
  if (!hasEip7702Signal) return issues; // plain factory — no flag
...
function detectBlobTxDependencies(code: string): PortabilityIssue[] {
  const issues: PortabilityIssue[] = [];
...
function detectGasOpcodesInfo(code: string): PortabilityIssue[] {
  const issues: PortabilityIssue[] = [];
...
function detectDelegatecallInfo(code: string): PortabilityIssue[] {
  const issues: PortabilityIssue[] = [];
 ...
 */
function detectSelfdestructInfo(code: string): PortabilityIssue[] {
  const issues: PortabilityIssue[] = [];
 ...
 */
export function formatMonadCompatibilityReport(
  report: MonadCompatibilityReport,
): string {
  const lines: string[] = [];
  ...
  lines.push(
    `${report.checks.passed}/${report.checks.total} anchor categories passed`,
  );
  ...
  if (anchorIssues.length > 0) {
    ...
    anchorIssues.forEach((issue) => {
      ...
      if (issue.line) lines.push(`  Line ${issue.line}: ${issue.code}`);
  ...
  if (report.summary.errors > 0) {
    lines.push(
      `❌ NOT READY: Fix ${report.summary.errors} anchor error(s) before deploying to Monad`,
    );
  } else if (report.summary.warnings > 0) {
    lines.push(
      `⚠️  CAUTION: Address ${report.summary.warnings} anchor warning(s) before deployment`,
    );
  } else {
