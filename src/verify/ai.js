...
function getClaudeClient() {
    if (!clientInstance) {
        ...
        if (!apiKey) {
            throw new Error("ANTHROPIC_API_KEY environment variable is missing in .env file!");
        }
 ...
 */
export async function generateSolCMCAssertions(contract) {
 ...
 */
export async function regenerateSolCMCAssertionsWithFeedback(contract, previousRaw, errorFeedback) {
    const client = getClaudeClient();
    const userPrompt = `Generate assertion JSON buat contract "${contract.contractName}" sesuai aturan di system prompt.`;
...
function parseAssertionsJSON(raw) {
    const cleaned = raw
        .replace(/^```json\s*/i, "")
        .replace(/^```\s*/i, "")
        .replace(/```\s*$/i, "")
    ...
    try {
        ...
        throw new Error(`Claude nggak balikin JSON valid: ${e.message}\nRaw: ${cleaned}`);
    }
    if (!parsed.functions || typeof parsed.functions !== "object") {
        throw new Error(`JSON dari Claude nggak punya field "functions" yang valid.\nRaw: ${cleaned}`);
    }
    return parsed;
}
/**
 ...
 */
export async function explainSolCMCFailure(contractCode, instrumentedCode, specificTarget) {
    const client = getClaudeClient();
