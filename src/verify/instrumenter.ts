...
// ============================================================
// Splicing tetap 100% deterministic (reuse bodyRange dari parser.ts).
// LLM (Groq) cuma nentuin ISI before/assert-nya, BUKAN posisi splice.
// ============================================================

export interface InjectedLineRange {
  start: number; // 1-indexed line, in FINAL instrumentedSource (header included)
  end: number;
...
export interface InstrumentWithLLMResult {
  ...
  injectedLineRanges: InjectedLineRange[]; // NEW — for solcmc.ts to classify CHC findings
}
 ...
 */
function findSuspiciousIdentifiers(
  code: string,
  ...
  while ((match = regex.exec(code)) !== null) {
    const ident = match[1];
    if (SOLIDITY_GLOBALS.has(ident)) continue;
    if (knownGood.has(ident)) continue;
    suspicious.add(ident);
  }
  return [...suspicious];
}

function buildKnownGoodSet(
  contract: ContractSummary,
  fn: FunctionSummary,
  beforeVarNames: string[],
): Set<string> {
  const known = new Set<string>();
  for (const sv of contract.stateVariables) known.add(sv.name);
  for (const p of fn.parameters) known.add(p.name);
  for (const f of contract.functions) known.add(f.name);
  for (const v of beforeVarNames) known.add(v);
  return known;
}
...
export function validateAssertions(
  ...
  for (const [rawFnName, set] of Object.entries(assertions.functions)) {
    ...
    if (!fn) {
      problems.push(
        `Function "${fnName}" yang direference di assertions nggak ada di contract "${contract.contractName}".`,
      );
      continue;
    }
    ...
    for (const w of fn.writes) {
      for (const call of fn.externalCalls) {
        if (!call.valueExpression || call.valueExpression === w.value) continue;
        for (const assertLine of set.beforeExternalCall || []) {
          if (!assertLine.includes(w.value)) continue;
          const withoutWriteValue = assertLine.split(w.value).join("");
          const stillHasTransferredValue = withoutWriteValue.includes(
            call.valueExpression,
          );
          if (!stillHasTransferredValue) {
            problems.push(
              `Function "${fnName}": assert di beforeExternalCall keliatan ke-mirror dari ` +
    ...
    for (const line of [
      ...
      if (suspicious.length > 0) {
        problems.push(
          `Function "${fnName}": identifier mencurigakan (nggak ketemu di state var/parameter/sibling function): ${suspicious.join(", ")} — di baris: "${line}"`,
        );
      }
    }
  }
 ...
 */
export function spliceAssertions(
  contract: ContractSummary,
  assertions: GeneratedAssertions,
): InstrumentWithLLMResult {
  ...
  const splicePoints: SplicePoint[] = [];
  ...
  for (const [rawFnName, set] of Object.entries(assertions.functions)) {
    ...
    if (!fn.bodyRange) {
      warnings.push(
        `Function "${fnName}" nggak punya bodyRange, instrumentasi di-skip.`,
      );
      continue;
    }

    const bodyText = contract.sourceCode.slice(
      fn.bodyRange[0],
      fn.bodyRange[1] + 1,
    );
    ...
    if (firstBraceRelativeIdx === -1 || lastBraceRelativeIdx === -1) {
      warnings.push(
        `Gagal nemuin brace buat function "${fnName}", instrumentasi di-skip.`,
      );
      continue;
    }
    ...
    if (set.beforeExternalCall && set.beforeExternalCall.length > 0) {
      if (fn.externalCallStatementRange) {
        ...
        warnings.push(
          `Function "${fnName}": Groq ngasih "beforeExternalCall" tapi function ini nggak kedetek punya external call — assert di-taro di akhir function sebagai fallback.`,
        );
        splicePoints.push({
  ...
  const ascendingByPosition = [...splicePoints].sort(
  ...
  const injectedLineRanges: InjectedLineRange[] = rawRanges.map((r) => ({
 ...
 */
export function checkSyntaxValid(instrumentedSource: string): {
  valid: boolean;
  error?: string;
} {
  try {
