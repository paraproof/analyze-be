...
function resolveTypeName(typeName: any): { type: string; isMapping: boolean } {
  if (!typeName) return { type: "unknown", isMapping: false };
...
function extractEventEmits(body: any) {
  const emits: { event: string; arguments: string[] }[] = [];
  if (!body) return emits;
  visit(body, {
...
function extractExternalCalls(body: any) {
  const calls: { type: string; target: string; valueExpression?: string }[] =
    [];
  ...
  visit(body, {
    FunctionCall(node: any) {
      ...
      if (expr?.type === "NameValueExpression") {
        ...
        if (nameValueList?.names && nameValueList?.arguments) {
          ...
          if (valueIdx !== -1 && nameValueList.arguments[valueIdx]) {
            valueExpression = expressionToString(
              nameValueList.arguments[valueIdx],
            );
          }
        }
      ...
      if (expr?.type === "MemberAccess") {
        ...
        if (
          ["call", "delegatecall", "staticcall", "transfer", "send"].includes(
            name,
          )
        ) {
          calls.push({
 ...
 */
function findExternalCallStatementRange(
  body: any,
): [number, number] | undefined {
  if (!body || !Array.isArray(body.statements)) return undefined;
...
function extractKeys(node: any): string[] {
  const keys: string[] = [];
...
function isAssignmentTarget(node: any, parent: any) {
  return (
...
function isIndexAccessBase(node: any, parent: any) {
  return parent?.type === "IndexAccess" && parent.base === node;
}

function collectPureAssignmentTargetNodes(body: any): Set<any> {
...
function addRead(
  ...
  keys: string[],
  seen: Set<string>,
  reads: { variable: string; keys: string[] }[],
) {
...
function extractStateAccess(
  body: any,
  stateVariables: StateVariableSummary[],
  parameters: string[],
) {
  const reads: { variable: string; keys: string[] }[] = [];
  ...
  visit(body, {
    Identifier(node: any, parent: any) {
      ...
      if (isIndexAccessBase(node, parent)) return;
      if (excludedFromReads.has(node)) return;
      if (stateNames.has(node.name)) addRead(node.name, [], seenReads, reads);
    },
    BinaryOperation(node: any) {
      ...
      if (node.left.type === "IndexAccess") {
        ...
        if (current.type === "Identifier" && stateNames.has(current.name)) {
          const writeVar: string = current.name;
          writes.push({
            variable: writeVar,
            keys,
    ...
    UnaryOperation(node: any) {
      ...
      if (
        node.subExpression?.type === "IndexAccess" &&
    ...
    IndexAccess(node: any, parent: any) {
      if (isAssignmentTarget(node, parent)) return;
      if (excludedFromReads.has(node)) return;
      let current = node;
      const keys: string[] = [];
...
function extractRequireConditions(body: any) {
  const requires: { expression: string }[] = [];
  if (!body) return requires;
  visit(body, {
    FunctionCall(node: any) {
      if (
...
function expressionToString(node: any): string {
  ...
  switch (node.type) {
    ...
    case "TupleExpression":
      return `(${(node.components || []).map((c: any) => (c ? expressionToString(c) : "")).join(", ")})`;
  ...
  for (const [fileName, rawCode] of Object.entries(files)) {
    ...
    try {
      ...
      for (const node of ast.children) {
        ...
        const stateVariables: StateVariableSummary[] = [];
        const functions: FunctionSummary[] = [];

        for (const raw of node.subNodes) {
          const subNode: any = raw;
          if (subNode.type === "StateVariableDeclaration") {
            for (const variable of subNode.variables) {
              ...
              const visibility =
                rawVisibility === "default" ? "internal" : rawVisibility;
              stateVariables.push({
        ...
        for (const raw of node.subNodes) {
          ...
          const returns = (subNode.returnParameters || []).map((p: any) => ({
            name: p.name || "unnamed",
            type: resolveTypeName(p.typeName).type,
          }));
          const access = extractStateAccess(
            subNode.body,
            stateVariables,
            parameters.map((p: any) => p.name),
          );
          const externalCalls = extractExternalCalls(subNode.body);
          const externalCallStatementRange = findExternalCallStatementRange(
            subNode.body,
          );
          const modifiers = (subNode.modifiers || []).map((m: any) => m.name);
          const emits = extractEventEmits(subNode.body);

          const bodyRange: [number, number] | undefined = subNode   .body?.range
