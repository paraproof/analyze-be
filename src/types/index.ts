// ============================================================
// Shared API Contract Types — /verify & /deploy
//
// File ini SOURCE OF TRUTH buat bentuk request/response JSON antara
// Frontend dan Backend. Lihat API_CONTRACT.md buat penjelasan lengkap
// tiap endpoint + contoh response.
//
// ATURAN (sesuai kesepakatan git workflow):
// - Backend 1 pegang hak edit utama file ini.
// - Butuh field baru? Request lewat PR kecil terpisah, jangan edit bebas.
// - Commit perubahan file ini terpisah dari commit fitur lain.
// ============================================================
...
export interface VerifyRequest {
  entry_point: string;
  files: Record<string, string>; // key = relative path, value = source code
}
...
export interface VerifyStatusResponse {
  ...
  // ada kalau status RUNNING (opsional, buat progress indicator)
  steps?: VerifyStep[];

  // ada kalau status PASSED atau FAILED
  entry_point?: string;
  invariants_checked?: InvariantCheckResult[];
  spec?: string; // cuma dikirim kalau PASSED
  duration_ms?: number;
  completed_at?: string;

  // ada kalau status ERROR
...
export interface DeployRequest {
  job_id: string; // job_id dari /verify yang statusnya WAJIB "PASSED"
  contract_name: string; // misal "MonadVault"
}
...
export interface DeploySuccessResponse {
  ...
  explorer_url: string;
  deployed_at: string;
  // hasil pencatatan ke VerificationRegistry (step ke-2, terpisah dari forge create)
...
export type DeployResponse =
  ...
  | DeployPartialSuccessResponse
  | DeployFailedResponse;

// ------------------------------------------------------------
// Generic error envelope (buat 400/404 di luar konteks job spesifik)
// ------------------------------------------------------------
...
export interface WriteOp {
  variable: string;
  keys: string[]; // [] kalau bukan mapping/array
  operation: "=" | "+=" | "-=" | "*=" | "/=" | string;
  value: string; // expression asli sebagai string, misal "amount"
}
