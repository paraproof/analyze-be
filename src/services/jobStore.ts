...
const jobStore = new Map<string, VerifyStatusResponse>();
...
const jobFilesStore = new Map<string, VerifyRequest>();

export function createJob(request: VerifyRequest): {
  jobId: string;
  createdAt: string;
} {
  ...
  const jobData: VerifyStatusResponse = {
    job_id: jobId,
    status: "PENDING",
...
export function getJob(jobId: string): VerifyStatusResponse | undefined {
  return jobStore.get(jobId);
}

export function getJobFiles(jobId: string): VerifyRequest | undefined {
  return jobFilesStore.get(jobId);
}

function updateStepStatus(
  jobId: string,
  stepName: VerifyStepName,
 ...
 */
async function runVerificationTask(jobId: string, request: VerifyRequest) {
  ...
  try {
    ...
    const mainContract =
      astSummaries.find((c) => c.fileName === request.entry_point) ??
      astSummaries[0];
    ...
    try {
      instrumentResult = await generateAndValidateInstrumentation(
        mainContract,
        2, // maxRetries — sengaja nggak gede, tiap retry = 1x call claude + 1x run solc (bisa puluhan detik)
        async (instrumentedSource) => {
          const check = await runSolCMC(
            jobDir,
            mainContract.fileName,
    ...
    if (instrumentResult.warnings.length > 0) {
      console.warn(
        `[Instrumentation Warnings] ${mainContract.contractName}:\n` +
    ...
    console.warn(
      `Instrumented source (${instrumentResult.attempts} attempt, ${instrumentResult.injectedAssertCount} assert) buat ${mainContract.contractName}:\n${instrumentResult.instrumentedSource}`,
    );
    ...
    const solcmcResult = await runSolCMC(
      jobDir,
      mainContract.fileName,
      mainContract.contractName,
      instrumentResult.instrumentedSource,
    ...
    if (!solcmcResult.passed) {
      ...
      const failedEntries = job.invariants_checked.filter(
        (inv) => inv.result === "FAILED",
      );

      const explanationByDetail = new Map<string, string>();
      for (const inv of failedEntries) {
        if (explanationByDetail.has(inv.detail || "")) continue; // dedupe kalau ada detail identik
        const explanation = await explainSolCMCFailure(
          mainFileContent,
          instrumentResult.instrumentedSource,
          { location: inv.function, message: inv.detail || "" },
        );
        explanationByDetail.set(inv.detail || "", explanation);
      }

      job.invariants_checked = job.invariants_checked.map((inv) =>
        inv.result === "FAILED"
